...
ObsQText = CharacterRange(
    ('\x01', '\x08'), ('\x0e', '\x1f')) | CharacterSet('\x0b\x0c')
...
QuotedPair = CharacterRange(('\x01', '\x09')) | CharacterSet(
    '\x0b\x0c') | CharacterRange(('\x0e', '\x7f'))
...
ObsRoute = '[' + ((AlphaNum + '-').many() + AlphaNum) + \
    ':' + (DText | ('\\' + QuotedPair)).many() + ']'
