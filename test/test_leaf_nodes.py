...
class TestCharacterRange(EasyRECase, TestCase):
    AUTO_TEST_RES = [CharacterRange(['B', 'F']), CharacterRange(
        ['1', '8']), CharacterRange(['A', 'y'])]

    AUTO_TEST_MATCHES = ['BCDEF', '12345678',
...
class TestNegativeCharacterRange(EasyRECase, TestCase):
    NEGATIVE = True
    AUTO_TEST_RES = [NegativeCharacterRange(['B', 'F']), NegativeCharacterRange(
        ['1', '8']), NegativeCharacterRange(['A', 'y'])]

    AUTO_TEST_MATCHES = ['AG', '09', 'z']
    AUTO_TEST_NOMATCH = ['BCDEF', '12345678',
                         'ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxy']


class TestCharacterSet(EasyRECase, TestCase):
    AUTO_TEST_RES = [CharacterSet('abc'), CharacterSet(
...
class TestNegativeCharacterSet(EasyRECase, TestCase):
    NEGATIVE = True
    AUTO_TEST_RES = [NegativeCharacterSet('abc'), NegativeCharacterSet(
        '+'), NegativeCharacterSet('.'), NegativeCharacterSet('[+.*]')]
    # Note that that actual format for AUTO_TEST_MATCHES is a sequence
    # of sequences of strings - we're abusin the fact that a string is
    # a sequence of single character strings here ...
